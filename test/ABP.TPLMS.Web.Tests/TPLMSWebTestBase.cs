...
{
    public abstract class TPLMSWebTestBase : AbpAspNetCoreIntegratedTestBase<Startup>
    {
        protected static readonly Lazy<string> ContentRootFolder;

        static TPLMSWebTestBase()
        {
        ...
        protected override IWebHostBuilder CreateWebHostBuilder()
        {
            return base
                ...
                .UseSetting(WebHostDefaults.ApplicationKey, typeof(TPLMSWebMvcModule).Assembly.FullName);
        }
        ...
        protected void UsingDbContext(Action<TPLMSDbContext> action)
        {
            using (var context = IocManager.Resolve<TPLMSDbContext>())
            {
                action(context);
                context.SaveChanges();
            }
        ...
        protected T UsingDbContext<T>(Func<TPLMSDbContext, T> func)
        {
            T result;
        ...
        protected async Task UsingDbContextAsync(Func<TPLMSDbContext, Task> action)
        {
            using (var context = IocManager.Resolve<TPLMSDbContext>())
            {
        ...
        protected async Task<T> UsingDbContextAsync<T>(Func<TPLMSDbContext, Task<T>> func)
        {
            T result;
